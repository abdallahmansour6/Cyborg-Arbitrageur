...
- `dispatch_base` shows the post-haircut size that was actually sent; `safe_ceiling_base` is the pre-haircut binary-search ceiling. `DEPTH_DISCOUNT` is a phantom-liquidity guard. The IOC limits travel in venue-native price units (`long_native` / `short_native`) because that's what the matching engine consumes.
